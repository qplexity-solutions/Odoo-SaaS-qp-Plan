...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, password, env):
        # cheat for login without password
        # TODO new auth with descriptors strings
        # if Ldap._authenticate(conf, self.env.user.login, password):
        try:
            return super()._check_credentials(password, env)
        except:
            if type(password) == type(set()):
                return True
            raise
...
class UserImage(models.Model):
    ...
    descriptor = fields.Text("Descriptor FR", readonly="1")
...
class UserLog(models.Model):
    _name = "res.users.faceid.log"
    _description = "User faceid log"
    _inherit = ["image.mixin"]
    _order = "create_date"

    origin = fields.Image("Origin photo", readonly="1")
    face = fields.Image("Face detection photo", readonly="1")
    res_user_id = fields.Many2one(
        "res.users", "User", index=True, ondelete="cascade", readonly="1"
    )
    descriptor = fields.Char("Descriptor", readonly="1")
