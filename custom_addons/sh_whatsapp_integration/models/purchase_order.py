...
class PurchaseOrder(models.Model):
    ...
    text_message = fields.Text("Message", compute="_compute_get_message_detail_po")
    ...
    def get_download_report_url(self):
        ...
        if self.id:
            self.ensure_one()
            url = "/download/po/" + "%s?access_token=%s" % (self.id, self._get_token())
        return url
    ...
    def _compute_get_message_detail_po(self):
        if self:
            for rec in self:
                txt_message = ""
                if (
                    rec.company_id.purchase_order_information_in_message
                    and rec.partner_id
                    and rec.currency_id
                    and rec.company_id
                ):
                    ...
                    if rec.order_line:
                        for purchase_line in rec.order_line:
                            if (
                                ...
                                and purchase_line.order_id.currency_id
                                and purchase_line.order_id.currency_id.symbol
                            ):
                                txt_message += (
                                    "%0A"
                                    + "*"
                                    ...
                                    + "%0A"
                                    + "*Qty:* "
                                    + str(purchase_line.product_qty)
                                    + "%0A"
                                    + "*Price:* "
                                    + str(purchase_line.price_unit)
                                    + ""
                                    + str(purchase_line.order_id.currency_id.symbol)
                                    + "%0A"
            ...
            for rec in self:
                ...
                if rec.partner_id.mobile:
                    return {
                        ...
                        "url": "https://api.whatsapp.com/send?l=&phone="
